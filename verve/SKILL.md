...
This skill uses the Membrane CLI (`npx @membranehq/cli@latest`) to interact with Verve Group. Membrane handles authentication and credentials refresh automatically — so you can focus on the integration logic rather than auth plumbing.
...
1. **Create a new connection:**
   ```bash
   npx @membranehq/cli@latest search verve --elementType=connector --json
   ```
...
1. **Check existing connections:**
   ...
   ```
   If a Verve Group connection exists, note its `connectionId`
