...
```bash
membrane connection patch <id> --data '{"connectionKey":"clio"}'
```

For multi-account setups (e.g. two separate Clio accounts), create the second one with `connect`:

```bash
membrane connect --integrationKey clio \
  --connectionKey clio-secondary --allowMultipleConnections
```
...
```bash
membrane act --connectionKey clio \
...
```bash
membrane act --key <action-key> --connectionKey clio \
  --input '<json>' --json
```
...
```bash
membrane action list --connectionKey clio --intent "describe what you want" --limit 10 --json
```
...
```bash
membrane act --key <action-key> --connectionKey clio --input '<json>' --json
```
...
```bash
membrane action create "describe what the action should do" --connectionKey clio --json
```
