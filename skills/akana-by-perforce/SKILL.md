...
1. **Create a new connection:**
   ```bash
   membrane search akana-by-perforce --elementType=connector --json
   ```
...
1. **Check existing connections:**
   ...
   ```
   If a Akana by Perforce connection exists, note its `connectionId`
...
When the available actions don't cover your use case, you can send requests directly to the Akana by Perforce API through Membrane's proxy. Membrane automatically appends the base URL to the path you provide and injects the correct authentication headers — including transparent credential refresh if they expire.
