...
This skill uses the [Membrane](https://getmembrane.com) CLI to interact with Microsoft Entra ID. Membrane handles authentication and credentials refresh automatically — so you can focus on the integration logic rather than auth plumbing.

**Always route through Membrane.** Don't hit Microsoft Entra ID's API directly. Use the `act` command (or the `/act` HTTP endpoint), which proxies every request through an authenticated connection.

**Never handle external-app credentials yourself.** OAuth tokens, API keys, refresh tokens — Membrane stores and manages them server-side. Pass a `connectionKey` (or `connectionId`) and the tools call Microsoft Entra ID on your behalf.
...
## Step 1 — Get a connection to Microsoft Entra ID

`connection ensure` finds an existing connection by URL/domain, or creates a new one if none matches. Use it as the default — it covers both cases in one call.

```bash
membrane connection ensure "https://microsoftentraid.com" --json
```
...
- **`CLIENT_ACTION_REQUIRED`** — the user or agent must do something to finish setup. The `clientAction` object describes what:
  ...
  - `clientAction.agentInstructions` (optional) — **follow these verbatim if present**. They tell the agent how to drive Microsoft Entra ID's side of the flow programmatically. Don't shortcut to "paste this URL" — the instructions exist because the agent is expected to handle it.
...
```bash
membrane connection patch <id> --data '{"connectionKey":"microsoft-entra-id"}'
```

For multi-account setups (e.g. two separate Microsoft Entra ID accounts), create the second one with `connect`:

```bash
membrane connect --integrationKey microsoft-entra-id \
  --connectionKey microsoft-entra-id-secondary --allowMultipleConnections
```
...
**Use this as the default for the very first call against a new Microsoft Entra ID connection.** It's the fastest way to confirm the connection works and to give the user a real response — no build step, no `BUILDING` state, no waiting.

```bash
membrane act --connectionKey microsoft-entra-id \
  --api '{"method":"GET","path":"/path/to/endpoint","query":{}}' \
  --json
```

Spec shape: `{ method, path, body?, headers?, query? }`. The Microsoft Entra ID base URL is prepended automatically. Auth is injected automatically.

Look up the right `path` and `method` from the Microsoft Entra ID API docs (or the Popular actions table below). Only escalate to a saved action (Step 3) if the user is going to run the same call repeatedly.
...
```bash
membrane act --key <action-key> --connectionKey microsoft-entra-id \
  --input '<json>' --json
```
...
```bash
membrane action list --connectionKey microsoft-entra-id --intent "describe what you want" --limit 10 --json
```
...
```bash
membrane act --key <action-key> --connectionKey microsoft-entra-id --input '<json>' --json
```
...
```bash
membrane action create "describe what the action should do" --connectionKey microsoft-entra-id --json
```
...
membrane action create \
  ...
  --integrationKey microsoft-entra-id --json
```
...
### Disconnected Microsoft Entra ID connection

Microsoft Entra ID's auth no longer works (token revoked, OAuth expired, credentials rotated). Read the connection state:
...
You get the mapped input, output, errors, plus the raw HTTP exchange with Microsoft Entra ID.
...
- **Let Membrane handle credentials.** Never ask the user for Microsoft Entra ID API keys or tokens. Connections handle auth lifecycle server-side.
