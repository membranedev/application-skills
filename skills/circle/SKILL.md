...
```bash
membrane connection patch <id> --data '{"connectionKey":"circle"}'
```

For multi-account setups (e.g. two separate Circle accounts), create the second one with `connect`:

```bash
membrane connect --integrationKey circle \
  --connectionKey circle-secondary --allowMultipleConnections
```
...
```bash
membrane act --connectionKey circle \
...
```bash
membrane act --key <action-key> --connectionKey circle \
  --input '<json>' --json
```
...
```bash
membrane action list --connectionKey circle --intent "describe what you want" --limit 10 --json
```
...
```bash
membrane act --key <action-key> --connectionKey circle --input '<json>' --json
```
...
```bash
membrane action create "describe what the action should do" --connectionKey circle --json
```
