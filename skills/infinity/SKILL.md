...
```bash
membrane connection ensure "https://startinfinity.com/" --json
```
...
```bash
membrane connection patch <id> --data '{"connectionKey":"infinity"}'
```

For multi-account setups (e.g. two separate Infinity accounts), create the second one with `connect`:

```bash
membrane connect --integrationKey infinity \
  --connectionKey infinity-secondary --allowMultipleConnections
```
...
```bash
membrane act --connectionKey infinity \
...
```bash
membrane act --key <action-key> --connectionKey infinity \
  --input '<json>' --json
```
...
```bash
membrane action list --connectionKey infinity --intent "describe what you want" --limit 10 --json
```
...
```bash
membrane act --key <action-key> --connectionKey infinity --input '<json>' --json
```
...
```bash
membrane action create "describe what the action should do" --connectionKey infinity --json
```
