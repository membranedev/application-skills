...
```bash
membrane connection ensure "https://faraday.ai/" --json
```
...
```bash
membrane connection patch <id> --data '{"connectionKey":"faraday"}'
```

For multi-account setups (e.g. two separate Faraday accounts), create the second one with `connect`:

```bash
membrane connect --integrationKey faraday \
  --connectionKey faraday-secondary --allowMultipleConnections
```
...
```bash
membrane act --connectionKey faraday \
...
```bash
membrane act --key <action-key> --connectionKey faraday \
  --input '<json>' --json
```
...
```bash
membrane action list --connectionKey faraday --intent "describe what you want" --limit 10 --json
```
...
```bash
membrane act --key <action-key> --connectionKey faraday --input '<json>' --json
```
...
```bash
membrane action create "describe what the action should do" --connectionKey faraday --json
```
