...
This skill uses the [Membrane](https://getmembrane.com) CLI to interact with Lightspeed R-Series. Membrane handles authentication and credentials refresh automatically — so you can focus on the integration logic rather than auth plumbing.

**Always route through Membrane.** Don't hit Lightspeed R-Series's API directly. Use the `act` command (or the `/act` HTTP endpoint), which proxies every request through an authenticated connection.

**Never handle external-app credentials yourself.** OAuth tokens, API keys, refresh tokens — Membrane stores and manages them server-side. Pass a `connectionKey` (or `connectionId`) and the tools call Lightspeed R-Series on your behalf.
...
## Step 1 — Get a connection to Lightspeed R-Series
...
- **`CLIENT_ACTION_REQUIRED`** — the user or agent must do something to finish setup. The `clientAction` object describes what:
  ...
  - `clientAction.agentInstructions` (optional) — **follow these verbatim if present**. They tell the agent how to drive Lightspeed R-Series's side of the flow programmatically. Don't shortcut to "paste this URL" — the instructions exist because the agent is expected to handle it.
...
```bash
membrane connection patch <id> --data '{"connectionKey":"lightspeed-r-series"}'
```

For multi-account setups (e.g. two separate Lightspeed R-Series accounts), create the second one with `connect`:

```bash
membrane connect --integrationKey lightspeed-r-series \
  --connectionKey lightspeed-r-series-secondary --allowMultipleConnections
```
...
**Use this as the default for the very first call against a new Lightspeed R-Series connection.** It's the fastest way to confirm the connection works and to give the user a real response — no build step, no `BUILDING` state, no waiting.

```bash
membrane act --connectionKey lightspeed-r-series \
  --api '{"method":"GET","path":"/path/to/endpoint","query":{}}' \
  --json
```

Spec shape: `{ method, path, body?, headers?, query? }`. The Lightspeed R-Series base URL is prepended automatically. Auth is injected automatically.

Look up the right `path` and `method` from the Lightspeed R-Series API docs (or the Popular actions table below). Only escalate to a saved action (Step 3) if the user is going to run the same call repeatedly.
...
```bash
membrane act --key <action-key> --connectionKey lightspeed-r-series \
  --input '<json>' --json
```
...
```bash
membrane action list --connectionKey lightspeed-r-series --intent "describe what you want" --limit 10 --json
```
...
```bash
membrane act --key <action-key> --connectionKey lightspeed-r-series --input '<json>' --json
```
...
```bash
membrane action create "describe what the action should do" --connectionKey lightspeed-r-series --json
```
...
membrane action create \
  ...
  --integrationKey lightspeed-r-series --json
```
...
### Disconnected Lightspeed R-Series connection

Lightspeed R-Series's auth no longer works (token revoked, OAuth expired, credentials rotated). Read the connection state:
...
You get the mapped input, output, errors, plus the raw HTTP exchange with Lightspeed R-Series.
...
- **Let Membrane handle credentials.** Never ask the user for Lightspeed R-Series API keys or tokens. Connections handle auth lifecycle server-side.
