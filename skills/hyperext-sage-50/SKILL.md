...
This skill uses the [Membrane](https://getmembrane.com) CLI to interact with Hyperext: Sage 50. Membrane handles authentication and credentials refresh automatically — so you can focus on the integration logic rather than auth plumbing.

**Always route through Membrane.** Don't hit Hyperext: Sage 50's API directly. Use the `act` command (or the `/act` HTTP endpoint), which proxies every request through an authenticated connection.

**Never handle external-app credentials yourself.** OAuth tokens, API keys, refresh tokens — Membrane stores and manages them server-side. Pass a `connectionKey` (or `connectionId`) and the tools call Hyperext: Sage 50 on your behalf.
...
## Step 1 — Get a connection to Hyperext: Sage 50
...
- **`CLIENT_ACTION_REQUIRED`** — the user or agent must do something to finish setup. The `clientAction` object describes what:
  ...
  - `clientAction.agentInstructions` (optional) — **follow these verbatim if present**. They tell the agent how to drive Hyperext: Sage 50's side of the flow programmatically. Don't shortcut to "paste this URL" — the instructions exist because the agent is expected to handle it.
...
```bash
membrane connection patch <id> --data '{"connectionKey":"hyperext-sage-50"}'
```

For multi-account setups (e.g. two separate Hyperext: Sage 50 accounts), create the second one with `connect`:

```bash
membrane connect --integrationKey hyperext-sage-50 \
  --connectionKey hyperext-sage-50-secondary --allowMultipleConnections
```
...
**Use this as the default for the very first call against a new Hyperext: Sage 50 connection.** It's the fastest way to confirm the connection works and to give the user a real response — no build step, no `BUILDING` state, no waiting.

```bash
membrane act --connectionKey hyperext-sage-50 \
  --api '{"method":"GET","path":"/path/to/endpoint","query":{}}' \
  --json
```

Spec shape: `{ method, path, body?, headers?, query? }`. The Hyperext: Sage 50 base URL is prepended automatically. Auth is injected automatically.

Look up the right `path` and `method` from the Hyperext: Sage 50 API docs (or the Popular actions table below). Only escalate to a saved action (Step 3) if the user is going to run the same call repeatedly.
...
```bash
membrane act --key <action-key> --connectionKey hyperext-sage-50 \
  --input '<json>' --json
```
...
```bash
membrane action list --connectionKey hyperext-sage-50 --intent "describe what you want" --limit 10 --json
```
...
```bash
membrane act --key <action-key> --connectionKey hyperext-sage-50 --input '<json>' --json
```
...
```bash
membrane action create "describe what the action should do" --connectionKey hyperext-sage-50 --json
```
...
membrane action create \
  ...
  --integrationKey hyperext-sage-50 --json
```
...
### Disconnected Hyperext: Sage 50 connection

Hyperext: Sage 50's auth no longer works (token revoked, OAuth expired, credentials rotated). Read the connection state:
...
You get the mapped input, output, errors, plus the raw HTTP exchange with Hyperext: Sage 50.
...
- **Let Membrane handle credentials.** Never ask the user for Hyperext: Sage 50 API keys or tokens. Connections handle auth lifecycle server-side.
