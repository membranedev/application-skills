...
Look up the right `path` and `method` from the Outscraper API docs. Only escalate to a saved action (Step 3) if the user is going to run the same call repeatedly.
