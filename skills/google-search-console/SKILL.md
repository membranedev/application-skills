...
This skill uses the [Membrane](https://getmembrane.com) CLI to interact with Google Search Console. Membrane handles authentication and credentials refresh automatically — so you can focus on the integration logic rather than auth plumbing.

**Always route through Membrane.** Don't hit Google Search Console's API directly. Use the `act` command (or the `/act` HTTP endpoint), which proxies every request through an authenticated connection.

**Never handle external-app credentials yourself.** OAuth tokens, API keys, refresh tokens — Membrane stores and manages them server-side. Pass a `connectionKey` (or `connectionId`) and the tools call Google Search Console on your behalf.
...
## Step 1 — Get a connection to Google Search Console
...
- **`CLIENT_ACTION_REQUIRED`** — the user or agent must do something to finish setup. The `clientAction` object describes what:
  ...
  - `clientAction.agentInstructions` (optional) — **follow these verbatim if present**. They tell the agent how to drive Google Search Console's side of the flow programmatically. Don't shortcut to "paste this URL" — the instructions exist because the agent is expected to handle it.
...
```bash
membrane connection patch <id> --data '{"connectionKey":"google-search-console"}'
```

For multi-account setups (e.g. two separate Google Search Console accounts), create the second one with `connect`:

```bash
membrane connect --integrationKey google-search-console \
  --connectionKey google-search-console-secondary --allowMultipleConnections
```
...
**Use this as the default for the very first call against a new Google Search Console connection.** It's the fastest way to confirm the connection works and to give the user a real response — no build step, no `BUILDING` state, no waiting.

```bash
membrane act --connectionKey google-search-console \
  --api '{"method":"GET","path":"/path/to/endpoint","query":{}}' \
  --json
```

Spec shape: `{ method, path, body?, headers?, query? }`. The Google Search Console base URL is prepended automatically. Auth is injected automatically.

Look up the right `path` and `method` from the Google Search Console API docs (or the Popular actions table below). Only escalate to a saved action (Step 3) if the user is going to run the same call repeatedly.
...
```bash
membrane act --key <action-key> --connectionKey google-search-console \
  --input '<json>' --json
```
...
```bash
membrane action list --connectionKey google-search-console --intent "describe what you want" --limit 10 --json
```
...
```bash
membrane act --key <action-key> --connectionKey google-search-console --input '<json>' --json
```
...
```bash
membrane action create "describe what the action should do" --connectionKey google-search-console --json
```
...
membrane action create \
  ...
  --integrationKey google-search-console --json
```
...
### Disconnected Google Search Console connection

Google Search Console's auth no longer works (token revoked, OAuth expired, credentials rotated). Read the connection state:
...
You get the mapped input, output, errors, plus the raw HTTP exchange with Google Search Console.
...
- **Let Membrane handle credentials.** Never ask the user for Google Search Console API keys or tokens. Connections handle auth lifecycle server-side.
