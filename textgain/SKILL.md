...
This skill uses the Membrane CLI (`npx @membranehq/cli@latest`) to interact with Textgain. Membrane handles authentication and credentials refresh automatically — so you can focus on the integration logic rather than auth plumbing.
...
### Connecting to Textgain

1. **Create a new connection:**
   ```bash
   npx @membranehq/cli@latest search textgain --elementType=connector --json
   ```
...
1. **Check existing connections:**
   ...
   ```
   If a Textgain connection exists, note its `connectionId`
