...
This skill uses the Membrane CLI (`npx @membranehq/cli@latest`) to interact with Oracle Field Service. Membrane handles authentication and credentials refresh automatically — so you can focus on the integration logic rather than auth plumbing.
...
### Connecting to Oracle Field Service

1. **Create a new connection:**
   ```bash
   npx @membranehq/cli@latest search oracle-field-service --elementType=connector --json
   ```
...
1. **Check existing connections:**
   ...
   ```
   If a Oracle Field Service connection exists, note its `connectionId`
