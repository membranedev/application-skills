...
This skill uses the Membrane CLI (`npx @membranehq/cli@latest`) to interact with Google reCAPTCHA. Membrane handles authentication and credentials refresh automatically — so you can focus on the integration logic rather than auth plumbing.
...
1. **Create a new connection:**
   ```bash
   npx @membranehq/cli@latest search google-recaptcha --elementType=connector --json
   ```
...
1. **Check existing connections:**
   ...
   ```
   If a Google reCAPTCHA connection exists, note its `connectionId`
