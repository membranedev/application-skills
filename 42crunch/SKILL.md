...
This skill uses the Membrane CLI (`npx @membranehq/cli@latest`) to interact with 42Crunch. Membrane handles authentication and credentials refresh automatically — so you can focus on the integration logic rather than auth plumbing.
...
1. **Create a new connection:**
   ```bash
   npx @membranehq/cli@latest search 42crunch --elementType=connector --json
   ```
...
1. **Check existing connections:**
   ...
   ```
   If a 42Crunch connection exists, note its `connectionId`
